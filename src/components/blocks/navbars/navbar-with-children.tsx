  ...
  const navItems = [
    {
      ...
      link: "jobs",
      children: [
        { name: "Web Development", link: "jobs" },
        { name: "Interface Design", link: "jobs/interface-design" },
        { name: "Search Engine Optimization", link: "jobs/search-engine-optimization" },
        { name: "Branding", link: "jobs/branding" },
        { name: "Content Creation", link: "jobs/content-creation" },
      ...
      <div className="hidden flex-1 flex-row items-center justify-center space-x-2 text-sm font-medium text-zinc-600 transition duration-200 hover:text-zinc-800 lg:flex lg:space-x-2">
        <Menu setActive={setActive} >
          {navItems.map((item: any) => (
           <MenuItem setActive={setActive} active={active} item={item.name} key={item.name} >
...
export const Menu = ({
  ...
  return (
    <nav
      ...
      className="relative flex justify-center space-x-4 rounded-full px-4 py-3 dark:bg-neutral-950"
    >
      {children}
    </nav>
  );
